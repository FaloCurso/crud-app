...
async function EmployeeData() {
    ...
    try {
        ...
        } else if (partes.length === 2 && partes[1].e_d === true) {
            entradaBtn.disabled = true;
            salidaBtn.disabled = true;
        ...
        salidaBtn.addEventListener("click", () => registrarSalida(p_id, fechaFormato, partes[0].entry_Time, partes[0].id));
...
async function registrarEntrada(employee_id, fechaFormato) {
    try {
        ...
        partesActivos.push(employee_id);
...
async function registrarSalida(employee_id, fechaFormato, entry_Time, id) {
    try {
        let horaSalida = obtenerHoraActual();
        let tiempo = calcularTiempo(entry_Time, horaSalida);

        const partes = await getRecordForUserAndDate(employee_id, fechaFormato);
        ...
        document.getElementById("entrada-btn").disabled = false;
        document.getElementById("salida-btn").disabled = true;
        partesActivos.splice(partesActivos.indexOf(employee_id), 1);
...
function mostrarParte(parte) {
    ...
    let tiempoTranscurrido = parte.departure_Time ? calcularTiempo(parte.entry_Time, parte.departure_Time) : calcularTiempo(parte.entry_Time, horaActual);
